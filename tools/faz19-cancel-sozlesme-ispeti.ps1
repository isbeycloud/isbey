<#
  ISBEY CLOUD — FAZ 19 `CancelDocument` SOZLESME ISPAT ARACI (TEST/SANDBOX)
  =========================================================================
  AMAC
    `CancelDocument` ucunun GERCEK sozlesmesini OLCMEK:
      S-1  Istek govdesi alan adlari: `uuid`/`cancelReason` mi, `Uuid`/`CancelReason` mi?
      S-2  Yanit govdesi semasi nedir?
      S-3  Yanitta is-seviyesi basari alani (IsSucceeded) var mi?

  NE YAPMAZ
    · BELGE GONDERMEZ. `SendDocument` CAGRILMAZ. Kontor YAKILMAZ.
  ...
  NASIL OLCER
    Ayni UYDURMA (var olmayan) belge kimligiyle IKI deney yapilir:
      Deney A: { uuid, cancelReason }
      Deney B: { Uuid, CancelReason }
    Hangisinin "zorunlu alan eksik" dedigini gormek dogru alan adini kanitlar.
    Var olmayan bir belge iptal EDILEMEZ — gercek iptal OLUSMAZ.
  ...
  CIKTI
    ...
    .verify-tmp\faz19-cancel-sozlesme-<zaman>.json  <- ham kanit (JWT maskeli)
#>
...
$Kayit = Join-Path $Tmp "faz19-cancel-sozlesme-$damga.txt"
...
try {
  ...
  Write-Host '  ISBEY CLOUD — FAZ 19 CancelDocument SOZLESME ISPAT ARACI' -ForegroundColor Cyan
  Write-Host '=' * 72 -ForegroundColor DarkCyan
  Yaz '  Bu arac PASS uretmez; yalniz SOZLESME olcer. Belge GONDERMEZ.' 'DarkGray'
  Yaz '  Kontor YAKMAZ. Canli ortama istek GITMEZ.' 'DarkGray'
  ...
  # tsx cagri yolu — ayni guvenilirlik sirasi (bkz. belge-akisi.ps1)
  ...
  $hedef  = 'server/tests/phase19CancelContractProbe.ts'
  ...
  $raw    = Join-Path $Tmp "faz19-cancel-raw-out-$damga.txt"
  $rawErr = Join-Path $Tmp "faz19-cancel-raw-err-$damga.txt"
  ...
  switch ($etiket) {
    'OLCULDU'         { Yaz '  SONUC: OLCULDU - sozlesme alanlari olculdu (bkz. docs/35)' 'Green' }
